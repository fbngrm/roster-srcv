# Roster change
This branch implements the requirements defined in the [task](https://github.com/) description.
...
```bash
cd $GOPATH/src/github.com/hellofreshdevtests
git clone git@github.com:hellofreshdevtests/fbngrm-golang-test.git
cd fbngrm-golang-test
git checkout dev
```
...
The configuration resides in the [docker-compose](https://github.com//docker-compose.yaml) file.
...
To add a player to a roster, we use a `PATCH` request since a partial update is performed to an existing resource.
The request payload needed to contain the new `roster-id`.
...
For configuring the service via environment variables use the docker-compose file.
...
### Database schema
The database is initialized the scripts contained in the `initdb` directory.
See the [documentation](https://hub.docker.com/_/postgres) for more information.

Rosters and players are referenced by a `1:n` relationship.
